...
fun allSources(category: String, country: String): List<Source> {
    val gson = Gson()
    ...
    val sourceList = gson.fromJson(jsonData, Array<Source>::class.java).toList()

    return sourceList.filter { it.category == category.lowercase() && it.country == country.lowercase() }
}